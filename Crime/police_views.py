...
from Crime.models import Criminals, Officer_Reg, PoliceReg, UserReg, Fir,Complaint, UserType, evidence, missing
...
class fir_Approved(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        m = Fir.objects.filter(status='fir_approved')
        print(m)
        context['m'] = m
        return context
...
class Police_officer_reg(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # pl = PoliceReg.objects.filter(login__last_name='1')
        p1 = PoliceReg.objects.get(login_id=self.request.user.id)
        context['p1'] = p1
        return context
