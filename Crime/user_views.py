...
from Crime.models import Criminals, Feedback, Officer_Reg, UserReg, PoliceReg, Complaint, add_evidence, missing
...
class contactpolice(LoginRequiredMixin,TemplateView):
    template_name = 'user/contact_police.html'
    login_url = '/'

    def get_context_data(self, **kwargs):
        context = super(contactpolice, self).get_context_data(**kwargs)
        staff = PoliceReg.objects.filter(login__last_name='1', login__is_staff='0')
        context['staff'] = staff
        return context
    
class View_Police(LoginRequiredMixin,TemplateView):
    template_name = 'user/view_police.html'
    login_url = '/'
    ...
    def get_context_data(self, **kwargs):
        id = self.request.GET['st']
        context = super(View_Police, self).get_context_data(**kwargs)
        m = Officer_Reg.objects.filter(station_id=id)
